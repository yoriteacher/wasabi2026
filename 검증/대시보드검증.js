  ...
  const s = {
    ...
    SpreadsheetApp: {
      getActiveSpreadsheet: () => ({
        getSheetByName: (이름) => {
          if (이름 === "발음기록") {
            return 줄들 ? { getDataRange: () => ({ getValues: () => 줄들 }) } : null;
          }
          if (이름 === "반배정") {
            return 반배정 ? { getDataRange: () => ({ getValues: () => 반배정 }) } : null;
          }
          return null;
        },
      }),
...
console.log("교사 대시보드 — 선택과목 합반 (출석부 배정)");

// 201 은 담임반 1반과 3반 학생이 섞인 합반이다
{
  const 합반줄들 = [
    제목줄,
    줄(1, 9, "20105", "가온", "こんにちは", 60),
    줄(2, 9, "20105", "가온", "こんにちは", 75),
    줄(1, 9, "20312", "나래", "こんにちは", 50),
    줄(2, 9, "20312", "나래", "こんにちは", 66),
    줄(1, 9, "20801", "다온", "こんにちは", 90),
  ];
  const 배정 = [
    ["학번", "수업반"],
    ["20105", "201"],
  ...
  // (가) 반배정 탭이 없으면 — 담임반으로 흩어진다 (지금까지의 동작)
  const 없을때 = vm.runInContext('대시보드자료("바른번호")',
    상자(합반줄들, "바른번호")).자료;
  확인("배정 탭이 없으면 담임반으로 나뉜다 (1·3·8반)",
    없을때.반들.map((ㅂ) => ㅂ.반).join(",") === "1,3,8");
  확인("배정 탭이 없으면 수업반씀=false", 없을때.수업반씀 === false);

  // (나) 반배정 탭이 있으면 — 출석부대로 묶인다
  const 있을때 = vm.runInContext('대시보드자료("바른번호")',
    상자(합반줄들, "바른번호", null, 배정)).자료;
  확인("배정 탭이 있으면 수업반으로 묶인다 (201·208)",
    있을때.반들.map((ㅂ) => ㅂ.반).join(",") === "201,208");
  확인("201 에 담임반이 다른 두 학생이 함께 들어간다",
    있을때.반들[0].반 === "201" && 있을때.반들[0].학생수 === 2);
  확인("208 은 한 명", 있을때.반들[1].학생수 === 1);
  확인("수업반씀=true", 있을때.수업반씀 === true);

  const 가온 = 있을때.학생들.filter((ㅅ) => ㅅ.이름 === "가온")[0];
  확인("학생에 수업반과 담임반이 둘 다 남는다",
    가온.반 === "201" && 가온.담임반 === "1" && 가온.합반 === true);

  const 다온 = 있을때.학생들.filter((ㅅ) => ㅅ.이름 === "다온")[0];
  확인("208 은 담임반과 같지만 배정에 있으므로 합반 표시", 다온.반 === "208");

  // (다) 배정표에 없는 학생은 담임반으로 남는다
  const 일부만 = vm.runInContext('대시보드자료("바른번호")',
    상자(합반줄들, "바른번호", null, [["학번", "수업반"], ["20105", "201"]])).자료;
  확인("배정에 없는 학생은 담임반으로 남는다",
    일부만.반들.map((ㅂ) => ㅂ.반).sort().join(",") === "201,3,8".split(",").sort().join(","));

  // (라) 제목이 달라도 찾는다
  const 다른제목 = vm.runInContext('대시보드자료("바른번호")',
    상자(합반줄들, "바른번호", null, [["번호", "반"], ["20105", "201"], ["20312", "201"], ["20801", "208"]])).자료;
  확인("배정 탭 제목이 달라도 읽는다",
    다른제목.반들.map((ㅂ) => ㅂ.반).join(",") === "201,208");

  // (마) 배정 탭이 제목만 있으면 무시
  const 빈배정 = vm.runInContext('대시보드자료("바른번호")',
    상자(합반줄들, "바른번호", null, [["학번", "수업반"]])).자료;
  확인("배정 탭이 비어 있으면 담임반으로 돌아간다", 빈배정.수업반씀 === false);
}
