 ...
 *
 * ※ 여기서 나오는 '반' 은 담임반입니다. 선택과목 합반이면 수업 반과 다릅니다.
 *   그래서 '반배정' 탭이 있으면 그것을 먼저 씁니다 (수업반읽기_ 참고).
 */
function 학번풀기_(학번) {
 ...
 *   학번으로 반을 읽으면 한 수업이 담임반 수만큼 조각납니다.
...
function 수업반읽기_() {
  ...
  var 학번칸 = 대시보드칸찾기_(제목, ['학번'], 0);
  var 반칸 = 대시보드칸찾기_(제목, ['수업반', '반'], 1);
...
/** 반 이름 차례. 숫자면 숫자 순, '?' 는 맨 뒤. (201·207·208 도 숫자라 그대로 맞습니다) */
function 반차례_(가, 나) {
  if (가 === '?') return 1;
  if (나 === '?') return -1;
